...
/* ------------------------------------------------------------
   PHASE 2 STUB — Square checkout
   This is intentionally not wired up yet because it needs your
   real Square credentials and a backend to talk to Square
   securely (a browser page can never hold a Square secret key).
   See README.md → "Connecting the real backend" for the plan:
   Cloudflare Worker <-> Square Payments API <-> Google Sheets.
   ------------------------------------------------------------ */
async function checkoutWithSquare() {
  alert(
    "Checkout isn't connected yet.\n\n" +
    "This button is ready for Square — once your Cloudflare Worker " +
    "and Square API keys are set up (see README.md), this function " +
    "will send the cart to your Worker, which creates the Square " +
    "payment, order, and shipping label."
  );
}
